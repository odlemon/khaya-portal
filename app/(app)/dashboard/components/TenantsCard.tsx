...
export default function TenantsCard() {
  ...
  const { getTenantCount } = useDashboardService();
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-200">
      <div className="flex items-center space-x-4">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-gray-500 mb-1">Total Tenants</p>
          <p className="text-3xl font-bold text-gray-900">
            {loading ? (
              <span className="text-gray-300 animate-pulse">—</span>
            ) : (
              tenantCount?.toLocaleString() || '0'
            )}
          </p>
        </div>
      </div>
